...
class Goods
{
    ...
    public function getGoodsInfo($goodsNo = null, $goodsField = null, $arrBind = null, $dataArray = false, $usePage = false)
    {
        ...
        if (empty($goodsNo) && empty($arrBind) && empty($goodsField)) {
            \Logger::channel('goods')->info("getGoodsInfo() params are null - goodsNo, arrBind, goodsField : " . json_encode(debug_backtrace()));
    ...
    public function getDisplayThemeInfo($dataSno = null, $dataArray = false)
    {
        ...
        if($kindType === 'event' && Request::isMobile() === true && trim($mobileThemeCd) !== ''){
            ...
            $eventMobileThemeData = $this->db->slave()->query_fetch($eventSQL, $arrBind2)[0];
    ...
    protected function setGoodsListInfo(&$getData, $imageType, $optionFl = false, $couponPriceFl = false, $viewWidthSize = null, $viewName = null, $brandFl = false, $linkUrl = null)
    {
        ...
        if(gd_is_plus_shop(PLUSSHOP_CODE_LISTMOUSEOVER) === true) {
            ...
            $strImageSQL = 'SELECT goodsNo,imageName,imageKind FROM ' . DB_GOODS_IMAGE . ' g  WHERE imageNo= 0  AND goodsNo IN ('.implode(',',$goodsBindQuery).')';
        } else {
            $arrBind = null;
            $this->db->bind_param_push($arrBind, 's', $imageType);
            foreach ($goodsArrBind as $goodsArrBindKey => $goodsArrBindVal) {
                if ($goodsArrBindKey == 0) continue;
                $this->db->bind_param_push($arrBind, 'i', $goodsArrBindVal);
            }
            $strImageSQL = 'SELECT goodsNo,imageName,imageKind FROM ' . DB_GOODS_IMAGE . ' g  WHERE imageKind = ?  AND goodsNo IN ('.implode(',',$goodsBindQuery).')';
        }
        ...
        if ($brandFl && $brandBindQuery) {
            ...
            $tmpData = $this->db->query_fetch($strSQLGlobal,$brandArrBind);
        ...
        if ($mallBySession) {
            ...
            $tmpData = $this->db->query_fetch($strSQLGlobal,$goodsArrBind);
        ...
        $getCateData = $this->db->slave()->query_fetch($strCateSQL, $goodsArrBind);
        foreach ($getCateData AS $cateInfo) {
        ...
        $getIconData = $this->db->slave()->query_fetch($strIconSQL, $goodsArrBind);
        foreach ($getIconData AS $iconInfo) {
        ...
        foreach ($getData as $key => &$val) {
            ...
            if ($setCouponUseType == 'y' && $couponPriceYN && $val['goodsPrice'] > 0 && empty($val['goodsPriceString']) === true && $this->goodsCouponDownList) {
                ...
                //쿠폰가 계산
                $couponSalePrice = $this->coupon->getGoodsCouponDownListPrice($val, $this->goodsCouponDownList);
    ...
    public function getGoodsLinkCnt($cateCd, $modeFl = 'only', $cateType = 'goods', $statusMode = 'admin')
    {
        if ($statusMode == 'admin') {
            ...
            if ($modeFl == 'only') {
                ...
                $data = $this->db->slave()->query_fetch($strSQL, $arrBind, $modeFl != 'only');
            } else {
                if ($cateType == 'goods') {
     ...
     *
     */
    public function setRevicwCount($goodsNo, $decreaseFl = false, $channel = null)
    {
        $arrBind = [];
        $this->db->bind_param_push($arrBind, 'i', $goodsNo);
        if ($decreaseFl) {
            ...
            $this->db->setModDtUse(false);
            $this->db->set_update_db(DB_GOODS, $strSet, 'goodsNo = ?', $arrBind);
            if($this->goodsDivisionFl) {
                $this->db->setModDtUse(false);
                $this->db->set_update_db(DB_GOODS_SEARCH, $strSet, 'goodsNo = ?', $arrBind);
            }
        } else {
            // 일반리뷰와 네이버리뷰 카운트를 분리
            $strSet = ($channel === 'naverpay') ? "naverReviewCnt = naverReviewCnt + 1" : "reviewCnt = reviewCnt + 1";
    ...
    public function getGoodsListSortLinkFl($cateType, $cateCd)
    {
        ...
        $this->db->bind_param_push($arrBind, 's', $cateCd);
        $this->db->bind_param_push($arrBind, 's', 'y');
        $result = $this->db->slave()->query_fetch($strSQL, $arrBind, false);
