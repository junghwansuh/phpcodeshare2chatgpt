...
var gd_goods_view = function() {
    ...
    var setAddBlindPrice = 0;
    var customParm = "cstpr";

	//designpix.kkamu
	var useBlindFl = "y";
	// var blindUnitPrice = 25000; 
	var useBlindOptionApplyFl = 'y';
    // var addBlindPrice;
    // var blindPrice;
    ...
    this.option_price_display = function(inputName) {
        ...
        if( useBlindFl ){
            ...
            $blind.each(function( i , tg){ 
                var $tg = $(tg);
                blindArr.push($tg.val());
            });

			var blindSize = $("#blindSize").val();

            if( ($blind.eq(0).val() == "" || $blind.eq(1).val() == "") || blindSize=="" ){
                alert("사이즈를 먼저 입력해주세요!");

                return false;
            }
            arrTmp_ori =  arrTmp[0];
            arrTmp[0] += blindArr[0] + blindArr[1];
        } 
		/* designpix.kkamu  자동 계산기 end*/
                    ...
                    if (i == data.nextKey) {
                        ...
                        for (var j = 0; j < data.nextOption.length; j++) {
                            if (data.optionViewFl[j] == 'y') {
                                ...
                                console.log('post mode: option_select');
                                console.log('addSelectOption = ', addSelectOption);

                                // 마지막 옵션의 경우 재고 체크 및 가격
                                if ((parseInt(data.nextKey) + 1) == parseInt(optionCnt)) {
                                    // 가격 출력여부
                                    console.log('옵션가격 출력 여부 분기 1', addSelectOption);

                                    if(parseInt(data.optionPrice[j]) !='0') {
                                        if(data.optionPrice[j] > 0 ) var addPlus = "+";
                                        else var addPlus = "";
                                        console.log("옵션가격 출력 여부 분기 2: data.optionPrice[j] > 0", addSelectOption);

                                        if (optionPriceFl == 'y' && optionPriceDiffFl == 'y') {
                                            addSelectOption	+= ' : ' + addPlus + gd_money_format(data.optionPrice[j].toString()) + '';
                                            console.log("옵션가격 출력 여부 분기 2-1: optionPriceFl == 'y' && optionPriceDiffFl == 'y'", addSelectOption);
                                        } else if (optionPriceFl == 'y' && optionPriceDiffFl == 'n' && defaultGoodsPrice != parseFloat(data.optionPrice[j])) {
                                            addSelectOption	+= ' (' + addPlus + gd_money_format(data.optionPrice[j].toString()) + ')';
                                            console.log("옵션가격 출력 여부 분기 2-2: optionPriceFl == 'y' && optionPriceDiffFl == 'n' && defaultGoodsPrice != parseFloat(data.optionPrice[j]", addSelectOption);
                                        }
                                    }
    ...
    this.goods_calculate = function(frmId, goodsFl, itemNo, goodsCnt) {
        console.log(':1086 this.goods_calculate START');

        console.log('gd_goods_view.js>this.goods_calculate>$addBlindPrice = ', $addBlindPrice);
        // console.log('gd_goods_view.js>this.goods_calculate>addBlindPrice = ', addBlindPrice);
        
        // var goodsPrice = parseFloat($(frmId + ' input[name="set_goods_price'+setOptNo+'"]').val());
        var goodsPrice = parseFloat($addBlindPrice);
        console.log('띵똥 ', goodsPrice);
        console.log('띠띵똥 ', goodsPrice);
        ...
        if (goodsFl) {
            ...
            var optionTotalPrice = (optionTextPrice) + (optionPrice) + ($addBlindPrice);



            /* designpix.kkamu  자동 계산기 */

// 			var blindPrice = 1000000;
// 			var addBlindPrice = $addBlindPrice + optionTotalPrice; 
//             console.log("gd_goods_view.js>$addBlindPrice = ", $addBlindPrice);
//             console.log("gd_goods_view.js>addBlindPrice = ", addBlindPrice);
// 			if( useBlindFl == 'y' ){
// 
// 				var blindPrice = goodsPrice;
// 
// 				if(useBlindOptionApplyFl=='y'){
// 					blindPrice += optionPrice;
// 				}
// 
// 				// var blindArr = $("#option_display_item_"+itemNo+" input[name='optionBlind[]']").val() ;
//                 var blindArr = $("blind").val() ;
// 
// 				var blind = blindArr.split(",");  
// 
// 
// 				var blindSize = blind[2];
// 
//                 var totalBlindPrice = $addBlindPrice;
// 				
// 				//var totalBlindPrice = Math.round( (blindSize * 2 * blindPrice)/100)  *100 ; 
// 				//가격 100원자리에서 올림
// 				// var totalBlindPrice = Math.ceil((Math.round((blindSize * 2 * blindPrice)/100)*100) / 1000 ) *1000; 
// 
// 				addBlindPrice = totalBlindPrice - goodsPrice;  
// 				if(useBlindOptionApplyFl=='y'){
// 					addBlindPrice -= optionPrice;  
// 				}
// 			}
// 
// 			/*org     $(frmId + ' .option_price_display_' + setOptNo + itemNo).html(gd_money_format(((optionTotalPrice + goodsPrice) * goodsCnt).toFixed(setDecimal))); */
            // $(frmId + ' .option_price_display_' + setOptNo + itemNo).html(gd_money_format(((optionTotalPrice + goodsPrice) * goodsCnt).toFixed(setDecimal)));
            $(frmId + ' .option_price_display_' + setOptNo + itemNo).html(gd_money_format(((optionTotalPrice) * goodsCnt).toFixed(setDecimal)));
// 
// 			/* designpix.kkamu  자동 계산기 end */	
			...
			if(setOptNo == "" || setOptNo == undefined) {
			/* designpix.kkamu  자동 계산기 */	
                console.log("옵션이 없는 경우", $addBlindPrice);
                ...
                console.log("옵션이 있는 경우", $addBlindPrice);
                /* designpix.kkamu  자동 계산기 */	
                ...
                $("#option_display_item_" + setOptNo + '_' + itemNo + " input[name='goodsPriceSum" + setOptNo + "[]']").val(((goodsPrice + $addBlindPrice) * goodsCnt).toFixed(setDecimal));
            }
/*org
            if(setOptNo == "" || setOptNo == undefined) {
                $('#option_display_item_' + itemNo + ' input[name="optionPriceSum[]"]').val((optionPrice * goodsCnt).toFixed(setDecimal));
...
*/
        ...
        console.log(':1086 this.goods_calculate END');
    };
    ...
    this.init = function(param) {
		...
		//designpix.kkamu
		useBlindFl = param.useBlindFl;
		// blindUnitPrice = param.blindUnitPrice;
		useBlindOptionApplyFl = param.useBlindOptionApplyFl;
        // $addBlindPrice = param.$addBlindPrice;
		//designpix.kkamu end 
        
        custom_param = param.customParm;
        setAddBlindPrice = param.setAddBlindPrice;
